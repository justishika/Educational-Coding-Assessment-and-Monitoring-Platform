...
export default function ScreenCapture({ 
  ...
  useEffect(() => {
    if (!userId || !isCodespaceActive) {
      ...
      console.log('📸 ScreenCapture: Not active - userId:', userId, 'isCodespaceActive:', isCodespaceActive);
      return;
    }
    
    console.log(`📸 ScreenCapture: Starting periodic capture for user ${userId} every ${interval}ms`);
    console.log(`📸 ScreenCapture: Codespace active, looking for iframe with title "Student Codespace"`);
    ...
    if (codespaceiframe) {
      console.log('📸 ScreenCapture: Found Student Codespace iframe:', codespaceiframe.src);
    } else {
      console.log('📸 ScreenCapture: Student Codespace iframe not found yet');
    }
    
    // Function to capture the screen
    const captureScreen = async () => {
      try {
        console.log(`📸 ScreenCapture: Starting capture attempt ${captureCount + 1} for user ${userId}`);
        
        // Check if there's a codespace iframe for metadata purposes
        const codespaceiframe = document.querySelector('iframe[title="Student Codespace"]') as HTMLIFrameElement;
        
        if (codespaceiframe) {
          console.log(`📸 ScreenCapture: Found codespace iframe: ${codespaceiframe.src}`);
        } else {
          console.log(`📸 ScreenCapture: No codespace iframe found`);
        }
        ...
        if (useScreenCapture && streamRef.current && videoRef.current) {
          try {
            ...
            console.log(`📸 Screenshot stored in MongoDB with method: ${screenshotData.metadata.captureMethod}`);
